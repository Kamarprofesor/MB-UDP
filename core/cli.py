...
class Command(Enum):
    ...
    INSTALL_MB-UDP = os.path.join(SCRIPT_DIR, 'mb-udp', 'install.sh')
    UNINSTALL_MB-UDP = os.path.join(SCRIPT_DIR, 'mb-udp', 'uninstall.sh')
    UPDATE_MB-UDP = os.path.join(SCRIPT_DIR, 'mb-udp', 'update.sh')
    RESTART_MB-UDP = os.path.join(SCRIPT_DIR, 'mb-udp', 'restart.sh')
    CHANGE_PORT_MB-UDP = os.path.join(SCRIPT_DIR, 'mb-udp', 'change_port.sh')
    CHANGE_SNI_MB-UDP = os.path.join(SCRIPT_DIR, 'mb-udp', 'change_sni.sh')
    GET_USER = os.path.join(SCRIPT_DIR, 'mb-udp', 'get_user.sh')
    ADD_USER = os.path.join(SCRIPT_DIR, 'mb-udp', 'add_user.sh')
    EDIT_USER = os.path.join(SCRIPT_DIR, 'mb-udp', 'edit_user.sh')
    RESET_USER = os.path.join(SCRIPT_DIR, 'mb-udp', 'reset_user.sh')
    REMOVE_USER = os.path.join(SCRIPT_DIR, 'mb-udp', 'remove_user.sh')
    SHOW_USER_URI = os.path.join(SCRIPT_DIR, 'mb-udp', 'show_user_uri.sh')
    IP_ADD = os.path.join(SCRIPT_DIR, 'mb-udp', 'ip.sh')
    MANAGE_OBFS = os.path.join(SCRIPT_DIR, 'mb-udp', 'manage_obfs.sh')
    TRAFFIC_STATUS = 'traffic.py'  # won't be call directly (it's a python module)
    UPDATE_GEO = os.path.join(SCRIPT_DIR, 'mb-udp', 'update_geo.py')
    LIST_USERS = os.path.join(SCRIPT_DIR, 'mb-udp', 'list_users.sh')
    SERVER_INFO = os.path.join(SCRIPT_DIR, 'mb-udp', 'server_info.sh')
    BACKUP_HYSTERIA = os.path.join(SCRIPT_DIR, 'mb-udp', 'backup.sh')
...
# region mb-udp menu options


@cli.command('install-mb-udp')
@click.option('--port', '-p', required=True, help='Port for MB-Udp', type=int, callback=validator.validate_port)
@click.option('--sni', '-s', required=False, default='bts.com', help='SNI for MB-Udp (default: bts.com)', type=str)
def install_mb-udp(port: int, sni: str):
    """
    Installs MB-Udp on the given port and uses the provided or default SNI value.
    """
    run_cmd(['bash', Command.INSTALL_MB-UDP.value, str(port), sni])


@cli.command('uninstall-mb-udp')
def uninstall_mb-udp():
    run_cmd(['bash', Command.UNINSTALL_MB-UDP.value])


@cli.command('update-mb-udp')
def update_mb-udp():
    run_cmd(['bash', Command.UPDATE_MB-UDP.value])


@cli.command('restart-mb-udp')
def restart_mb-udp():
    run_cmd(['bash', Command.RESTART_MB-UDP.value])


@cli.command('change-mb-udp-port')
@click.option('--port', '-p', required=True, help='New port for MB-Udp', type=int, callback=validator.validate_port)
def change_mb-udp_port(port: int):
    run_cmd(['bash', Command.CHANGE_PORT_MB-UDP.value, str(port)])

@cli.command('change-mb-udp-sni')
@click.option('--sni', '-s', required=True, help='New SNI for MB-Udp', type=str)
def change_mb-udp_sni(sni: str):
    run_cmd(['bash', Command.CHANGE_SNI_MB-UDP.value, sni])
...
def manage_obfs(remove, generate):
    """Manage 'obfs' in MB-Udp configuration."""
