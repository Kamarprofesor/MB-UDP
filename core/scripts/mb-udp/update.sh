...
echo "Starting the update process for MB-Udp..."
...
echo "Downloading and installing the latest version of MB-Udp..."
...
if [ $? -ne 0 ]; then
    ...
    python3 "$CLI_PATH" restart-mb-udp > /dev/null 2>&1
    exit 1
fi
...
python3 "$CLI_PATH" restart-mb-udp > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "Error: Failed to restart MB-Udp service."
    exit 1
fi

echo "MB-Udp has been successfully updated."
echo ""
exit 0
