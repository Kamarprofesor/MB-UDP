...
EOF
    ...
    if systemctl is-active --quiet hysteria-server.service; then
        echo -e "${cyan}MB-Udp${NC} has been successfully installed."
    else
        echo -e "${red}Error:${NC} hysteria-server.service is not active."
        exit 1
    fi
    
    chmod +x /etc/hysteria/core/scripts/mb-udp/user.sh
    chmod +x /etc/hysteria/core/scripts/mb-udp/kick.sh

    (crontab -l ; echo "*/1 * * * * /bin/bash -c 'source /etc/hysteria/mb-udp_venv/bin/activate && python3 /etc/hysteria/core/cli.py traffic-status' >/dev/null 2>&1") | crontab -
    (crontab -l ; echo "0 3 */3 * * /bin/bash -c 'source /etc/hysteria/mb-udp_venv/bin/activate && python3 /etc/hysteria/core/cli.py restart-mb-udp' >/dev/null 2>&1") | crontab -
    (crontab -l ; echo "0 */6 * * * /bin/bash -c 'source /etc/hysteria/mb-udp_venv/bin/activate && python3 /etc/hysteria/core/cli.py backup-hysteria' >/dev/null 2>&1") | crontab -
    (crontab -l ; echo "*/1 * * * * /etc/hysteria/core/scripts/mb-udp/kick.sh >/dev/null 2>&1") | crontab -

}

if systemctl is-active --quiet hysteria-server.service; then
    echo -e "${red}Error:${NC} MB-Udp is already installed and running."
    echo
    echo "If you need to update the core, please use the 'Update Core' option."
else
    echo "Installing and configuring MB-Udp..."
    install_hysteria "$1"
    echo -e "\n"
    ...
    if systemctl is-active --quiet hysteria-server.service; then
        ...
        echo -e "${red}Error:${NC} MB-Udp service is not active. Please check the logs for more details."
    fi
fi
