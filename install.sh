...
git clone https://github.com/Kamarprofesor/MB-UDP /etc/hysteria

cd /etc/hysteria
python3 -m venv mb-udp_venv
source /etc/hysteria/mb-udp_venv/bin/activate
pip install -r requirements.txt &> /dev/null && echo "Install Python requirements ✅"

if ! grep -q "alias mbudp='source /etc/hysteria/mb-udp_venv/bin/activate && /etc/hysteria/menu.sh'" ~/.bashrc; then
    echo "alias mbudp='source /etc/hysteria/mb-udp_venv/bin/activate && /etc/hysteria/menu.sh'" >> ~/.bashrc
    source ~/.bashrc
fi
sleep 5
cd /etc/hysteria
