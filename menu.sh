...
mb-udp_install_handler() {
    ...
    python3 $CLI_PATH install-mb-udp --port "$port" --sni "$sni"
...
mb-udp_change_port_handler() {
    ...
    python3 $CLI_PATH change-mb-udp-port --port "$port"
}

mb-udp_change_sni_handler() {
    while true; do
    ...
    python3 $CLI_PATH change-mb-udp-sni --sni "$sni"
...
display_main_menu() {
    clear
    tput setaf 7 ; tput setab 4 ; tput bold ; printf '%40s%s%-12s\n' "◇───────────ㅤ🚀ㅤWelcome To MB-Udp Managementㅤ🚀ㅤ───────────◇" ; tput sgr0
    echo -e "${LPurple}◇──────────────────────────────────────────────────────────────────────◇${NC}"
...
# Function to display the MB-Udp menu
display_mb-udp_menu() {
    clear
    echo -e "${LPurple}◇──────────────────────────────────────────────────────────────────────◇${NC}"

    echo -e "${yellow}                   ☼ MB-Udp Menu ☼                   ${NC}"

    echo -e "${LPurple}◇──────────────────────────────────────────────────────────────────────◇${NC}"

    echo -e "${green}[1] ${NC}↝ Install and Configure MB-Udp"
...
# Function to handle MB-Udp menu options
mb-udp_menu() {
    clear
    local choice
    while true; do
        ...
        read -r choice
        case $choice in
            1) mb-udp_install_handler ;;
            2) mb-udp_add_user_handler ;;
            3) mb-udp_edit_user ;;
            4) mb-udp_reset_user_handler ;;
            5) mb-udp_remove_user_handler  ;;
            6) mb-udp_get_user_handler ;;
            7) mb-udp_list_users_handler ;;
            8) python3 $CLI_PATH traffic-status ;;
            9) mb-udp_show_user_uri_handler ;;
            0) return ;;
            *) echo "Invalid option. Please try again." ;;
        esac
        echo
...
display_advance_menu() {
    ...
    echo -e "${cyan}[8] ${NC}↝ Change Port MB-Udp"
    echo -e "${cyan}[9] ${NC}↝ Change SNI MB-Udp"
    echo -e "${cyan}[10] ${NC}↝ Manage OBFS"
    echo -e "${cyan}[11] ${NC}↝ Change IPs(4-6)"
    echo -e "${cyan}[12] ${NC}↝ Update geo Files"
    echo -e "${cyan}[13] ${NC}↝ Restart MB-Udp"
    echo -e "${cyan}[14] ${NC}↝ Update Core MB-Udp"
    echo -e "${red}[15] ${NC}↝ Uninstall MB-Udp"
    echo -e "${red}[0] ${NC}↝ Back to Main Menu"
    echo -e "${LPurple}◇──────────────────────────────────────────────────────────────────────◇${NC}"
    echo -ne "${yellow}➜ Enter your option: ${NC}"
}
...
advance_menu() {
    ...
    while true; do
        ...
        case $choice in
            ...
            6) singbox_handler ;;
            7) normalsub_handler ;;
            8) mb-udp_change_port_handler ;;
            9) mb-udp_change_sni_handler ;;
            10) obfs_handler ;;
            11) edit_ips ;;
            12) python3 $CLI_PATH update-geo ;;
            13) python3 $CLI_PATH restart-mb-udp ;;
            14) python3 $CLI_PATH update-mb-udp ;;
            15) python3 $CLI_PATH uninstall-mb-udp ;;
            0) return ;;
            *) echo "Invalid option. Please try again." ;;
        esac
        echo
